...
class TimeUtil(object):
    ...
    def __init__(self, **keywords):
        ...
        '''
        self.timeformat = keywords.get('timeformat', '%Y-%m-%d_%H-%M-%S')
        self.tzinfo = pytz.timezone(keywords.get('timezone', 'UTC'))
    ...
    def anytype2timestr(self, src):
        ...
        if isinstance(src, str):
            ...
            dst = self.unixtime2timestr(int(src))
        else:
    ...
    def anytype2datetime(self, src):
        ...
        if isinstance(src, str):
            ...
            dst = self.unixtime2datetime(int(src))
        else:
            logger.error('input data type is not supported.', type(src))
            dst = src
    ...
    def get_day_of_week_sin_cos(self, src):
        ...
        rel_ut = src_ut - bow
        if rel_ut > len_week or rel_ut < 0:
            from IPython import embed
            embed()
        dst = atleast_1d(
            [
    ...
    def now(self, format='timestr'):
        dt_now = self.__datetime_naive2aware(datetime.now())
        if format == 'timestr':
...
# -----------------------------------------------------------------------------
def get_time_range_pairs(
        time_bgn, time_end, dur_sec, apply_hour_bgn=None, apply_hour_end=None):
    '''
    @argv
    ...
    while tb < dend:
        ...
        if apply_hour_bgn is not None or apply_hour_end is not None:
            ahb = datetime(
                tb.year, tb.month, tb.day, apply_hour_bgn, tzinfo=tu.tzinfo)
            ahe = datetime(
                tb.year, tb.month, tb.day, apply_hour_end, tzinfo=tu.tzinfo)
